...
function renderBars(container, rows, expanded) {
  ...
  tooltip.className = "pressure-total-service-custom-tooltip";
  tooltip.hidden = true;
  container.append(tooltip);

  const header = document.createElement("div");
  header.className = "pressure-total-service-header";
  header.innerHTML = '<span></span><span class="pressure-total-service-col-label">No. Companies</span>';
  ...
  visibleRows.forEach((row) => {
    ...
    barWrap.addEventListener("mouseenter", (e) => {
      tooltip.textContent = `Total pressure score: ${formatTotal(row.total)}`;
