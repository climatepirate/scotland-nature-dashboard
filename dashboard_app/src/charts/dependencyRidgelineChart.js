...
const DEFAULT_SERVICE = "Air filtration";
...
async function loadRidgelineData() {
  if (!ridgelineDataPromise) {
    ridgelineDataPromise = fetchDashboardDataJson("dependency_ridgeline_index.json", "dependency ridgeline index")
      .then((payload) => {
        const lookup = new Map();
        const services = new Map();
        (payload.records || []).forEach((record) => {
          const key = `${record.service_key}||${record.local_authority_code}||${record.coarse_category}`;
          lookup.set(key, {
          ...
          services.set(record.service_key, record.service_label);
        });
...
function resolveServiceKey(data, selectedService) {
  const selected = normalizeServiceName(selectedService || DEFAULT_SERVICE);
  if (data.serviceLabelsByKey.has(selected)) {
    return selected;
  }

  const fallbackEntry = Array.from(data.serviceLabelsByKey.keys())[0] || selected;
  return fallbackEntry;
}

function resolveCategoriesForState(data, state, serviceKey) {
  ...
  const unclassifiedRecord = data.lookup.get(`${serviceKey}||${localAuthority}||${UNCLASSIFIED}`);
...
export function initDependencyRidgelineChart() {
  if (ridgelineChartInitialized) {
    return;
  }

  const svg = document.getElementById("dependency-ridgeline-chart");
  const subtitle = document.getElementById("dependency-ridgeline-subtitle");
  const status = document.getElementById("dependency-ridgeline-status");
  const tooltip = document.getElementById("dependency-ridgeline-tooltip");
  ...
  const render = async () => {
    showStatus("Loading dependency distribution…");
    hideTooltip();
    clearSvg(svg);
    ...
    try {
      ...
      const records = categories.map((category) => {
        const record = data.lookup.get(`${serviceKey}||${localAuthority}||${category}`);
        return {
      ...
      if (!hasAnyData) {
        showStatus("No company-level dependency scores available for the current filters.");
        return;
      }
      ...
      const curveData = records.map((record) => {
        const cacheKey = `${serviceKey}||${localAuthority}||${record.coarseCategory}`;
        return {
          ...record,
      ...
      axisTitle.textContent = "Dependency score";
      ...
      curveData.forEach((item, index) => {
        ...
        hoverRect.addEventListener("mousemove", (event) => {
          ...
          tooltip.innerHTML = `
            ...
            <div>Dependency score: ${formatScore(score)}</div>
      ...
      showStatus(`Unable to render dependency ridgeline: ${error?.message || error}`);
    }
  };
  ...
  subscribe((nextState, prevState) => {
    if (
      ...
      || nextState.selectedDependency !== prevState.selectedDependency
    ) {
      scheduleRender();
    }
  });
